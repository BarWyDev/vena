...
export default function EligibilityCards({ eligibility, initialSelectedType }: Props) {
  ...
  const resolvedType: DonationType = selectedType ?? "whole_blood";
  const exportDate = eligibility[resolvedType];
    ...
    <>
      ...
      {exportDate && (
        <a
          ...
          className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-medium text-white backdrop-blur-xl transition-colors hover:bg-white/15"
        >
